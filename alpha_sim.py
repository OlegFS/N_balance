...
g_= [5.4,5.5,5.6,5.7,5.8,5.9,6.0]#[4.7,4.8,4.9,5.0,5.1,5.2,5.3]
eta_ = [0.85]
d =[3.5]
...
for i in range(len(values)):
    ...
    eta = values[i][1]
    print('delta %s'%(i))
    simulation = Path('sim/alpha_delta/brunel_esp_ex_%s_g_%s_test01dt_delta.npy'%(g,eta))
...
for i in range(len(values)):
    ...
    g = values[i][0]
    eta = values[i][1]
    simulation = Path('sim/alpha_delta/brunel_esp_ex_%s_g_%s_test01dt_alpha.npy'%(g,eta))
    if not  simulation.exists() or repeat ==True:
